...
function App() {
  ...
  // Permission Check: Can this user see salaries?
  const canSeeSalary = currentUser && [
    "Admin", "General Manager", "Finance & Accountant Manager", "Finance & Accountant Employee"
  ].includes(currentUser.role);

  // Permission Check: Can this user manage staff (Add/Edit/Reset)?
  const canManageStaff = currentUser && [
    "Admin", "General Manager", "Human Resource Manager"
  ].includes(currentUser.role);
  ...
  async function changeMyPassword() {
    ...
    if (!error) { alert("Password updated!"); setShowPassModal(false); }
  }
  ...
  async function resetUserPassword(emp) {
    ...
    alert("Reset to phone number successful.");
    fetchEmployees();
  }
  ...
  async function handleSubmit(e) {
    ...
    if (!error) { setEditingId(null); fetchEmployees(); resetForm(); alert("Saved!"); }
    setLoading(false);
  }
  ...
  const exportToCSV = () => {
    const headers = `ID,Name,Role,Designation,Phone,NID,${canSeeSalary ? 'Salary,' : ''}Blood,Site\n`;
    const rows = filteredEmployees.map(e => 
      `${e.employee_id},${e.name},${e.role},${e.designation},${e.phone_number},${e.nid_number},${canSeeSalary ? e.basic_salary + ',' : ''}${e.blood_group},${e.site_location}`
    ).join("\n");
  ...
  const printID = (emp) => {
    ...
    win.document.write(`
      <html><head><style>
        ...
        .card { width: 350px; height: 210px; border: 3px solid #003366; border-radius: 12px; padding: 20px; background: #fff; position: relative; }
    ...
    return (
      <div style={loginBoxStyle}>
        ...
        <form onSubmit={handleLogin}>
          <input style={inputStyle} placeholder="Employee ID" value={loginForm.id} onChange={e => setLoginForm({...loginForm, id: e.target.value})} required />
    ...
    <div style={{ padding: '15px', maxWidth: '1400px', margin: 'auto', fontFamily: 'sans-serif' }}>
      <div style={headerStyle}>
        <div>
          ...
          <small>Logged in: <strong>{currentUser.name}</strong> ({currentUser.role})</small>
        </div>
        <div>
      ...
      {canManageStaff && (
        <form onSubmit={handleSubmit} style={formBoxStyle}>
          <h3 style={{ marginTop: 0 }}>{editingId ? 'Edit Profile' : 'New Staff Registration'}</h3>
          <div style={gridStyle}>
          ...
          <button type="submit" disabled={loading} style={{...buttonStyle, marginTop: '20px'}}>{loading ? 'Processing...' : 'Save Record'}</button>
          {editingId && <button onClick={resetForm} style={{...buttonStyle, background: '#666', marginTop: '5px'}}>Cancel</button>}
        </form>
      )}
      ...
      {showPassModal && (
        <div style={modalOverlayStyle}>
          <div style={modalContentStyle}>
            ...
            <input type="password" style={inputStyle} placeholder="New Password" onChange={e => setNewPass(e.target.value)} />
