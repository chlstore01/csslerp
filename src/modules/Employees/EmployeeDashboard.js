...
// We changed "function App()" to this so it receives the "currentUser" package
export default function EmployeeDashboard({ currentUser }) {
  ...
  async function handleSubmit(e) {
    ...
    // If no password provided, default to phone number or '123456'
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1200px', margin: 'auto', fontFamily: 'sans-serif' }}>
      <div style={headerStyle}>
        <h2 style={{ color: '#003366' }}>CSSL Employee Directory</h2>
        <div>
          {/* SECURITY GATE: Only Admin/HR can Add */}
          {canManageEmployees && (
          ...
          <button onClick={() => fetchEmployees()} style={{...btnStyle, background: '#6c757d', color: '#fff', width: 'auto', padding: '10px 20px'}}>Refresh</button>
        </div>
      </div>
            ...
            {employees.filter(e => e.name?.toLowerCase().includes(searchTerm.toLowerCase()) || e.employee_id?.includes(searchTerm)).map(emp => (
              <tr key={emp.employee_id} style={{ borderBottom: '1px solid #eee' }}>
                ...
                <td style={tdStyle}>
                  ...
                  {/* SECURITY GATE: Only Admin/HR can Edit */}
                  {canManageEmployees && (
                    <button onClick={() => handleEdit(emp)} style={actionBtn('#ffc107', '#000')}>Edit</button>
                  )}
                  
                  {/* SECURITY GATE: Only Admin can Delete */}
                  {isAdmin && (
        ...
        <div style={modalOverlay}>
          <div style={modalContent}>
            ...
            <form onSubmit={handleSubmit} style={gridStyle}>
              ...
              {/* Password field only manageable by Admin/HR or on creation */}
              <div><label style={label}>Login Password</label><input style={inputStyle} type="text" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} /></div>
